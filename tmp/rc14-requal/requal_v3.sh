...
run_arm() {
  ...
  for i in $(seq 1 "$CNT"); do
    ...
    [ "$MODE" = a ] && cp "$D/events.jsonl" "$BASE/evidence/a-run${i}-events.jsonl" 2>/dev/null
    [ -s "$D/ev.jq" ] || fail "$MODE run $i: no docker events captured"
    off() { jq -sr --arg a "$1" --arg s "$2" '[.[] | select(.Action == $a) | select(($s == "") or ((.Actor.Attributes?.signal? // "") == $s)) | .timeNano] | first // empty' "$D/ev.jq" 2>/dev/null; }
    ...
    [ "$SDIE" != null ] || fail "$MODE run $i: no die event timestamp"
    [ "$SFIN" != null ] || fail "$MODE run $i: no FinishedAt timestamp"
    [ "$S15" != null ] || fail "$MODE run $i: no SIGTERM(15) kill event"
